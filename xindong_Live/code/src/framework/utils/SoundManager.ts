...
class SoundManager extends SingleClass {
    ...
    public playEffect(soundName: string, startTime: number = 0, loops: number = 1) {
        ...
        if (sound == null) {
            ...
            if (sound != null) {
                ...
                //从远程获取
                RES.getResByUrl(soundName, (sound:egret.Sound)=>{
                    if(sound){
                        this.soundList[soundName] = sound;
                        sound.type = egret.Sound.EFFECT;
                        return sound.play(startTime, loops)
    ...
    public playBGM(bgmName: string, startTime: number = 0, loops: number = Number.MAX_VALUE) {
        //禁止播放背景音乐，则返回
        if(this.allowPlayBGM == false || bgmName == null && bgmName == ""){
            return;
        }
        //首先停止背景音乐
        this.stopBGM();
        //播放背景音乐
        var sound: egret.Sound = this.soundList[bgmName];
        if(sound){
            sound.type = egret.Sound.MUSIC;
            if(this.bgmChannel == null) {
                this.bgmChannel = sound.play(0,Number.MAX_VALUE);
                this.bgmChannel && (this.bgmChannel.volume = 0.5);
            }
        //背景音乐不存在
        }else{
            //从RES中获取
            sound = RES.getRes(bgmName);
            if(sound){
                this.soundList[bgmName] = sound;
                sound.type = egret.Sound.MUSIC;
                if(this.bgmChannel == null) {
                    this.bgmChannel = sound.play(0,Number.MAX_VALUE);
                    this.bgmChannel && (this.bgmChannel.volume = 0.5);
            ...
            //从远程获取
                ...
                RES.getResByUrl(bgmName, (sound:egret.Sound)=>{
                    if(sound){
                        this.soundList[bgmName] = sound;
                        sound.type = egret.Sound.MUSIC;
                        if(this.bgmChannel == null) {
