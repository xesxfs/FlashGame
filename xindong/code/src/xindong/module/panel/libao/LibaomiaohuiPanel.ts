...
class LibaomiaohuiPanel extends BasePanel {
    ...
    public constructor() {
        super();
        this.skinName = "LibaomiaohuiPanelSkin"
    }
    ...
    /**购买 */
    private buyBtnTouch() {
        console.log("keybuy", App.DataCenter.keybuy);
        let obj = App.DataCenter.keybuy.miaohui;
         //Android支付,sdk自己下单         
        if(App.DeviceUtils.IsAndroid && App.DeviceUtils.IsNative){
            ...
            let param = { gid: 0 }
            param.gid = App.DataCenter.keybuy.miaohui.id;
    ...
    private buyBack(data) {
        if (data.code == 200) {
            this.close();
            if (App.DataCenter.keybuy.miaohui.coin_type == "rmb") {
                App.NativeBridge.sendPay(data);
            }
            if (App.DataCenter.keybuy.miaohui.coin_type == "diamond") {
                Tips.info("购买成功")
                App.DataCenter.keybuy.miaohui.hasbuy = true;
            }
