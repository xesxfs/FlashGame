...
class LibaoCDPanel extends BasePanel {
    ...
    /**购买 */
    private buyBtnTouch() {
        let obj = App.DataCenter.keybuy.wulengque;
         //Android支付,sdk自己下单         
        if(App.DeviceUtils.IsAndroid && App.DeviceUtils.IsNative){
            ...
            let param = { gid: 0 }
            param.gid = App.DataCenter.keybuy.wulengque.id;
    ...
    private buyBack(data) {
        if (data.code == 200) {
            this.close();
            if (App.DataCenter.keybuy.wulengque.coin_type == "rmb") {
                App.NativeBridge.sendPay(data);
            }
            if (App.DataCenter.keybuy.wulengque.coin_type == "diamond") {
                Tips.info("购买成功")
                App.DataCenter.keybuy.wulengque.hasbuy = true;
            }
